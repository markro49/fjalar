...
void handle_possible_exit(MCEnv* mce, IRJumpKind jk) {
  if (Ijk_Ret == jk) {
    ...
    FJALAR_DPRINTF("[handle_possible_exit] curFuncPtr: %p, currentAddr: %x\n", curFuncPtr, (UInt)currentAddr);
...
void fjalar_pre_clo_init()
{
  ...
  VG_(memset)(FunctionExecutionStateStack, 0, VG_N_THREADS * FN_STACK_SIZE * sizeof FunctionExecutionStateStack[0][0]);
